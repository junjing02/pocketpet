...
- **Unmet-need cues:** whichever stat is at/below `LOW_STAT_THRESHOLD` (20) drives a distinct CSS animation and status-badge label directly on the sprite — hungry (shake), tired (nod), dirty (itch), sad (droop) — computed live by `lowStatMood()`, one at a time by priority (hunger → energy → hygiene → happiness), so care needs are visible without reading the stat bars. Sick/sleeping/egg override it (their own badge takes priority).
...
- **Landing page:** static marketing page (`index.html`) with a wandering pet preview, mini-game previews, a full evolution tree (one shared egg at the root branching into one row per species, hatchling → adult) below the games, and a Play button into the app (`app.html`)
...
- **One fixed `DOT_SIZE` (5px) for every stage, not a per-stage size.** It used to scale up with the stage (`STAGE_DOT_SIZE`, 2.4-7.5px across the app's history) so the pet visibly grew — but since the outline is always exactly 1 dot wide, growing the dot size grew the outline's absolute thickness right along with it, so a hatchling's border looked thin and an adult's looked chunky, inconsistent with each other and with the thin hairline borders everywhere else in the UI. Growth now comes entirely from each stage's silhouette occupying more cells (a hatchling is ~9 dots wide, an adult ~23 — plenty of size contrast on its own), so the outline reads as the same stroke width at every stage.
...
- The "Your Pets" screen (`data-screen="pet-picker"`), reached via the ⇄ icon on the main pet screen's topbar, is the single hub for everything pet-roster-related — it's where Rename, Reset, and Release moved to from Settings. All three are deliberately scoped to **the active pet only**, not per-card — the list itself only ever offers Select (to change which one's active); management actions live in one consistent place below it, plus:
  ...
  - **Release**: calls `deletePet` on the active pet, re-fetches, then auto-selects the oldest remaining one and runs the normal switch pipeline — or drops straight to the "name your egg" screen if that was the *last* pet. This is the answer to "I'm stuck with the same 3 pets forever" — `MAX_PETS` is still enforced, but it's no longer a hard ceiling once you're willing to let your active one go.
